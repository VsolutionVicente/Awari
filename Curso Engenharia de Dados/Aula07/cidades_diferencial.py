...
path = '/home/awari/app/aula-07/ingest/diferencial/cidades.csv'
...
# Caminhos o S3/MinIO para um arquivo chamado status.csv e cidades.csv
# o Status CSV contem a data hora do ultimo usuário importado, por isso ele é importante
key_status = "cidades/diferencial/status.csv"
key_cidades = "cidades/diferencial/cidades.csv"
...
cidades_filtrados_por_data = df[df['criado_em'] > str(status_datetime_serie.iloc[0])]
cidades_df = pd.concat([cidades_df, cidades_filtrados_por_data], ignore_index=True)
print(cidades_df)

# Pega a data do usuário mais recente
most_recent_date = cidades_df['criado_em'].max()
status_df.iloc[0] = cidades_df['criado_em'].max()
print(status_df)
